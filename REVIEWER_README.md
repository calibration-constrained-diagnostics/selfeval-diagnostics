...
File MD5 checksums for every redistributed input are in `metadata/checksums.txt`. The Croissant metadata at `metadata/croissant.json` has been validated against the MLCommons Croissant 1.0 schema; see `metadata/validation_report.md` (or rerun `python scripts/validate_croissant.py`).
